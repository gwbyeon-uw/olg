...
def rbf(D, scale=1.0):
    ...
    D_mu = D_mu[None,:]
    D_sigma = scale * (D_max - D_min) / D_count
...
def make_full_graph(xyz, pair, idx):
    ...
    G.edata['rel_pos'] = (xyz[b,j,:] - xyz[b,i,:]).detach().float() # no gradient through basis function

    return G, pair[b,i,j][...,None]

def make_topk_graph(xyz, pair, idx, top_k=64, kmin=32, topk_incl_local=False, eps=1e-8):
    '''
    Input:
        - xyz: current backbone cooordinates (B, L, 3, 3)
        - pair: pair features from Trunk (B, L, L, E)
