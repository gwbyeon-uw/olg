...
def get_init_xyz(xyz_t):
    ...
    B, T, L = xyz_t.shape[:3]
    init = INIT_CRDS.to(xyz_t.device).reshape(1,1,1,27,3).repeat(B,T,L,1,1) + torch.rand(B, T, L, 1, 3, device=xyz_t.device)*5.0
...
def get_init_xyz_compl(xyz_t, same_chain):
    # input: xyz_t (B, T, L, 14, 3)
    # ouput: xyz (B, T, L, 14, 3)
    B, T, L = xyz_t.shape[:3]
    init = INIT_CRDS.to(xyz_t.device).reshape(1,1,1,27,3).repeat(B,T,L,1,1) + torch.rand(B, T, L, 1, 3, device=xyz_t.device)*5.0
